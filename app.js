...
$('#download').onclick=async()=>{try{
  ...
  const fecha=$('#fecha').value?new Date($('#fecha').value+'T12:00:00'):new Date();
  ...
  const blob=await wb.outputAsync({type:'blob'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`INTEGRACION MASTERWEB ${$('#fecha').value||'CONCILIACION'}.xlsx`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
